...
CSV_PATH     = os.path.join(MATERIAL_DIR, "ship.csv")
...
# Experimentos: todas las combinaciones de aug_level y val_split
# scratch_noaug: sin augmentation, 3 val_splits
# scratch_aug:   aug_level 1.0 y 1.5, 3 val_splits = 6 configs
# pretrained_noaug: sin augmentation, 3 val_splits
# pretrained_aug:   aug_level 1.0 y 1.5, 3 val_splits = 6 configs
CONFIGS = [
    # --- Scratch sin augmentation (3 configs) ---
    {"name": "scratch_noaug_val10",    "model_type": "scratch",    "augment": False, "aug_level": 0.0, "val_split": 0.1, "train_cfg": TRAIN_CONFIG},
    {"name": "scratch_noaug_val20",    "model_type": "scratch",    "augment": False, "aug_level": 0.0, "val_split": 0.2, "train_cfg": TRAIN_CONFIG},
    {"name": "scratch_noaug_val30",    "model_type": "scratch",    "augment": False, "aug_level": 0.0, "val_split": 0.3, "train_cfg": TRAIN_CONFIG},

    # --- Scratch con augmentation (6 configs: 2 aug_level x 3 val_split) ---
    {"name": "scratch_aug10_val10",    "model_type": "scratch",    "augment": True,  "aug_level": 1.0, "val_split": 0.1, "train_cfg": TRAIN_CONFIG},
    {"name": "scratch_aug10_val20",    "model_type": "scratch",    "augment": True,  "aug_level": 1.0, "val_split": 0.2, "train_cfg": TRAIN_CONFIG},
    {"name": "scratch_aug10_val30",    "model_type": "scratch",    "augment": True,  "aug_level": 1.0, "val_split": 0.3, "train_cfg": TRAIN_CONFIG},
    {"name": "scratch_aug15_val10",    "model_type": "scratch",    "augment": True,  "aug_level": 1.5, "val_split": 0.1, "train_cfg": TRAIN_CONFIG},
    {"name": "scratch_aug15_val20",    "model_type": "scratch",    "augment": True,  "aug_level": 1.5, "val_split": 0.2, "train_cfg": TRAIN_CONFIG},
    {"name": "scratch_aug15_val30",    "model_type": "scratch",    "augment": True,  "aug_level": 1.5, "val_split": 0.3, "train_cfg": TRAIN_CONFIG},

    # --- Pretrained sin augmentation (3 configs) ---
    {"name": "pretrained_noaug_val10", "model_type": "pretrained", "augment": False, "aug_level": 0.0, "val_split": 0.1, "train_cfg": PRETRAINED_CONFIG},
    {"name": "pretrained_noaug_val20", "model_type": "pretrained", "augment": False, "aug_level": 0.0, "val_split": 0.2, "train_cfg": PRETRAINED_CONFIG},
    {"name": "pretrained_noaug_val30", "model_type": "pretrained", "augment": False, "aug_level": 0.0, "val_split": 0.3, "train_cfg": PRETRAINED_CONFIG},

    # --- Pretrained con augmentation (6 configs: 2 aug_level x 3 val_split) ---
    {"name": "pretrained_aug10_val10", "model_type": "pretrained", "augment": True,  "aug_level": 1.0, "val_split": 0.1, "train_cfg": PRETRAINED_CONFIG},
    {"name": "pretrained_aug10_val20", "model_type": "pretrained", "augment": True,  "aug_level": 1.0, "val_split": 0.2, "train_cfg": PRETRAINED_CONFIG},
    {"name": "pretrained_aug10_val30", "model_type": "pretrained", "augment": True,  "aug_level": 1.0, "val_split": 0.3, "train_cfg": PRETRAINED_CONFIG},
    {"name": "pretrained_aug15_val10", "model_type": "pretrained", "augment": True,  "aug_level": 1.5, "val_split": 0.1, "train_cfg": PRETRAINED_CONFIG},
    {"name": "pretrained_aug15_val20", "model_type": "pretrained", "augment": True,  "aug_level": 1.5, "val_split": 0.2, "train_cfg": PRETRAINED_CONFIG},
    {"name": "pretrained_aug15_val30", "model_type": "pretrained", "augment": True,  "aug_level": 1.5, "val_split": 0.3, "train_cfg": PRETRAINED_CONFIG},
]


def run_config(cfg, run_dir, all_results):
    print(f"\n{'='*60}\nConfig: {cfg['name']}\n{'='*60}")
...
def generate_final_report(all_results, configs, run_dir):
    ...
    def robustness(name): return np.mean([std(name, m) for m in metrics])  # lower = more robust

    groups = {
        "scratch_noaug":    [n for n in all_names if cfg_by_name[n]["model_type"] == "scratch"    and not cfg_by_name[n]["augment"]],
        "scratch_aug":      [n for n in all_names if cfg_by_name[n]["model_type"] == "scratch"    and     cfg_by_name[n]["augment"]],
        "pretrained_noaug": [n for n in all_names if cfg_by_name[n]["model_type"] == "pretrained" and not cfg_by_name[n]["augment"]],
        "pretrained_aug":   [n for n in all_names if cfg_by_name[n]["model_type"] == "pretrained" and     cfg_by_name[n]["augment"]],
    }
    group_titles = {
        "scratch_noaug":    "Scratch sin augmentation",
        "scratch_aug":      "Scratch con augmentation",
    ...
    L("=" * 70)
    L("INFORME FINAL DE EXPERIMENTOS - SHIP DETECTION (Tarea 2)")
    L("=" * 70)
    L("")
    ...
    for gkey, gnames in groups.items():
        ...
        # Analisis por val_split dentro del grupo
        ...
        # Analisis por aug_level (solo grupos con augmentation)
    ...
    L("")
    L("=" * 70)
    L("3. COMPARACION ENTRE LOS 4 GRUPOS")
    L("=" * 70)
    L("")
    ...
    L(f"  {'Grupo':<22} {'Acc media':>10} {'F1 media':>10} {'AUC media':>10} {'Robustez':>10}")
    L("  " + "-" * 65)
    for gkey in groups:
        g = group_avg[gkey]
        L(f"  {group_titles[gkey]:<22} {g['accuracy']:>10.4f} {g['f1']:>10.4f} {g['auc']:>10.4f} {g['robustez']:>10.4f}")
    L("")
    ...
    scratch_acc = np.mean([group_avg[g]["accuracy"] for g in ["scratch_noaug", "scratch_aug"]])
    pretrained_acc = np.mean([group_avg[g]["accuracy"] for g in ["pretrained_noaug", "pretrained_aug"]])
    scratch_rob = np.mean([group_avg[g]["robustez"] for g in ["scratch_noaug", "scratch_aug"]])
    pretrained_rob = np.mean([group_avg[g]["robustez"] for g in ["pretrained_noaug", "pretrained_aug"]])

    # Aug vs NoAug
    aug_acc   = np.mean([group_avg[g]["accuracy"] for g in ["scratch_aug", "pretrained_aug"]])
    noaug_acc = np.mean([group_avg[g]["accuracy"] for g in ["scratch_noaug", "pretrained_noaug"]])
    aug_rob   = np.mean([group_avg[g]["robustez"] for g in ["scratch_aug", "pretrained_aug"]])
    noaug_rob = np.mean([group_avg[g]["robustez"] for g in ["scratch_noaug", "pretrained_noaug"]])

    L(f"  Scratch vs Pretrained (accuracy media):  scratch={scratch_acc:.4f}  pretrained={pretrained_acc:.4f}")
    L(f"    => {'Pretrained' if pretrained_acc >= scratch_acc else 'Scratch'} da mejor accuracy media")
    L(f"  Scratch vs Pretrained (robustez):        scratch={scratch_rob:.4f}  pretrained={pretrained_rob:.4f}")
    L(f"    => {'Pretrained' if pretrained_rob <= scratch_rob else 'Scratch'} es mas robusto (std menor)")
    L("")
    ...
    # Aug level global (solo aug configs)
    ...
    L("=" * 70)
    L("")
    best_overall = winner_acc  # usamos accuracy como criterio principal
    ...
    # Redactar valoracion final en lenguaje natural
    verdict_model = "pretrained" if pretrained_acc >= scratch_acc else "scratch"
    ...
    L(f"    - Usar modelo {verdict_model} da mejores resultados en promedio.")
    L(f"    - La data augmentation {verdict_aug} a mejorar la accuracy.")
...
def main():
    run_dir, run_name = create_run_dir(BASE_DIR, "ship")
    print(f"Run directory: {run_dir}")

    # Guardar config con info sobre experimentos
    config_info = {
        "description": "Experiments to test validation split sizes and augmentation levels",
        "metrics_tracked": ["validation_loss_stability", "std_across_runs", "robustness"],
        "guidance": "Higher std indicates model is sensitive - increase augmentation. High val_loss std indicates sudden changes - may indicate overfitting.",
    }

    save_hyperparams(run_dir, "ship", CONFIGS, extra={
        "csv": CSV_PATH,
    ...
    all_results = {}
    for cfg in CONFIGS:
        all_results[cfg["name"]] = run_config(cfg, run_dir, all_results)
    ...
    plot_configs_comparison(all_results, global_results_dir, "ship")
